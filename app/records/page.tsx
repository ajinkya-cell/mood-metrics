...
export default function SentimentRecordsPage() {
  ...
  const renderVerdictMatrix = () => {
    ...
    return (
      <DoubleBezel className="p-6 h-full flex flex-col justify-between">
        ...
        <div className="mt-4 border-t border-white/5 pt-3 font-mono text-[9px] text-zinc-500 flex justify-between">
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Bullish
          </span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-zinc-500" /> Neutral
          </span>
          <span className="flex items-center gap-1">
            <span className="w-1.5 h-1.5 rounded-full bg-rose-500" /> Bearish
          </span>
        </div>
      </DoubleBezel>
    );
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-6 py-12 select-none overflow-hidden pb-20 font-mono">
      {/* Header section */}
      <div className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-white/5 pb-8">
        <div>
          <SectionEyebrow icon={<Database size={12} className="text-emerald-400" />}>
            QUANTITATIVE LEDGER
          </SectionEyebrow>
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/dashboard"
            className="flex items-center gap-1 px-4 py-2 border border-white/5 bg-[#0C0C0E] hover:bg-white/5 text-zinc-400 hover:text-white rounded-lg text-xs transition-all"
          >
            <ArrowLeft size={12} /> BACK TERMINAL
          </Link>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <GlowCard accent={stats.avgScore > 0.2 ? "emerald" : stats.avgScore < -0.2 ? "rose" : "zinc"} className="p-6">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-[10px] text-zinc-500">(-100 to +100)</span>
          </div>
          <div className="mt-3 text-[10px] text-zinc-400 font-sans border-t border-white/5 pt-2">
            Blended average across all matching parsed signals.
          </div>
        </GlowCard>

        {/* AI Confidence */}
        <GlowCard accent="zinc" className="p-6">
          <div className="text-[10px] text-zinc-500 uppercase tracking-wider mb-2">
            AI CLASSIFIER CONFIDENCE
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-4xl font-extrabold font-sans text-white">
              {Math.round(stats.avgConfidence * 100)}%
            </span>
            <span className="text-[10px] text-zinc-500">ACCURACY BIAS</span>
          </div>
          <div className="mt-3 text-[10px] text-zinc-400 font-sans border-t border-white/5 pt-2">
          ...
          <DoubleBezel className="p-6">
            <div className="flex justify-between items-baseline mb-6 border-b border-white/5 pb-4">
              <h2 className="text-sm font-bold text-white flex items-center gap-1.5">
                <ChartLineUp className="text-emerald-400" />
            ...
            {isLoading ? (
              <div className="flex h-64 items-center justify-center font-mono text-zinc-500 text-xs animate-pulse">
      ...
      <DoubleBezel className="p-6 mb-8 bg-[#0C0C0E]/40">
        ...
        <form onSubmit={handleSearchSubmit} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
          ...
          <div className="md:col-span-3 flex flex-col gap-2">
            <label className="text-[10px] text-zinc-500 font-bold uppercase">ASSET COMPONENT</label>
            <select
          ...
          <div className="md:col-span-2 flex flex-col gap-2">
            <label className="text-[10px] text-zinc-500 font-bold uppercase">SENTIMENT LABEL</label>
            <select
              value={selectedLabel}
              onChange={(e) => {
          ...
          {/* Source Selector */}
          <div className="md:col-span-2 flex flex-col gap-2">
            <label className="text-[10px] text-zinc-500 font-bold uppercase">INGEST SOURCE</label>
            <select
          ...
          <div className="md:col-span-1">
            <button
              type="submit"
              className="w-full bg-emerald-500 hover:bg-emerald-400 text-black text-xs font-bold py-2 px-3 rounded-lg flex items-center justify-center gap-1 transition-all"
            >
              FILTER
            </button>
          </div>
        ...
        {isLoading ? (
          <div className="space-y-4">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="h-28 border border-white/5 rounded-2xl bg-zinc-950/20 animate-pulse"
              />
            ))}
          </div>
        ) : records.length === 0 ? (
          ...
          records.map((item) => {
            ...
            return (
              <DoubleBezel key={item.id} className="w-full">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/5 pb-2 mb-3 text-[10px] text-zinc-600">
                  <div className="flex flex-wrap items-center gap-2">
                    {isReddit ? (
                      <RedditLogo size={12} className="text-rose-500" />
                    ) : (
                    ...
                    <span className="px-1.5 py-0.5 rounded bg-zinc-900 border border-white/5 text-zinc-400 font-bold">
                      {item.tickerSymbol}
                    </span>
                ...
                <div className="mt-3 pt-3 border-t border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-[10px]">
                  <button
                  ...
                  {item.postUrl && (
                    ...
                    >
                      REDIRECT NODE SOURCE <ArrowSquareOut size={8} />
                    </a>
                  )}
                </div>
      ...
      {!isLoading && records.length > 0 && (
        <div className="flex items-center justify-between mt-8 border-t border-white/5 pt-6 text-xs text-zinc-500">
          <span>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={page === 0}
              className={`px-3 py-1.5 rounded border border-white/5 flex items-center gap-1 transition-all ${
                page === 0
                  ? "opacity-40 cursor-not-allowed"
                  : "bg-black hover:bg-white/5 text-zinc-300 hover:text-white"
              }`}
            >
