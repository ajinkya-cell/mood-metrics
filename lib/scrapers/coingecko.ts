...
import { rawPosts, scrapeCache, Ticker } from "../db/schema";
...
type CoinGeckoCoinData = {
    ...
    community_data : {
        ...
        tiwtter_followers: number
    }
    developer_data : {
        stars : number;
        forks : number ;
        pull_request_contributers : number ;
...
async function geckoFetch<T>(path :string , attempt = 1): Promise<T> {
    ...
    try {
        ...
        if(res.status === 429){
            ...
            console.warn(`[Coin Gecko] Rate limit - waiting ${waitMs/1000} s`)
            await sleep(waitMs)
...
export async function fetchCoinPrice(coingeckoId : string) : Promise<CoinPrice | null> {
    try {
        ...
        return {
            ...
            twitterFollowers : data.community_data?.tiwtter_followers ?? 0
        };
    } catch (error) {
        console.warn(`[CoinGecko] fetchCoinPrice failed for ${coingeckoId} :` , error)
        return null
    }
}
...
export async function scrapeCoinGecko(ticker : Ticker) : Promise<number>{
    if (!ticker.coingeckoId){
        console.log(`[CoinGecko] No coingecko_id for this ${ticker.symbol}`);
        return 0;
    }
    ...
    try {
        ...
        const msg = error instanceof Error ? error.message : "Unknown error from coingecko #169"
